    ...
    async def _test():
        ...
        async with async_session() as session:
            ...
            users = [
                ...
                MockUser(name="Charlie", email="charlie@example.com"),
            ]
...
def test_get_all_with_pagination():
    ...
    async def _test():
        ...
        async with async_session() as session:
            ...
            # Create 5 users
            users = [
                MockUser(name=f"User{i}", email=f"user{i}@example.com")
                for i in range(5)
            ]
            session.add_all(users)
            await session.flush()
            ...
            # Get first 2
            repo = MockUserRepository(session)
            result = await repo.get_all(offset=0, limit=2)
            assert len(result) == 2
            
            # Get next 2
            result = await repo.get_all(offset=2, limit=2)
            assert len(result) == 2
            
            # Get remaining 1
            result = await repo.get_all(offset=4, limit=2)
            assert len(result) == 1
    ...
    async def _test():
        ...
        async with async_session() as session:
            ...
            users = [
                MockUser(name="Alice", email="alice@example.com"),
                MockUser(name="Bob", email="bob@example.com"),
                MockUser(name="Alice", email="alice2@example.com"),
                MockUser(name="Charlie", email="charlie@example.com"),
            ]
            session.add_all(users)
            await session.flush()
            ...
            # Filter by name
            repo = MockUserRepository(session)
            result = await repo.get_all(name="Alice")
            assert len(result) == 2
            assert all(u.name == "Alice" for u in result)
            
            # Filter by email
            result = await repo.get_all(email="bob@example.com")
            assert len(result) == 1
...
def test_update_non_existent():
    ...
    async def _test():
        ...
        async with async_session() as session:
            ...
            repo = MockUserRepository(session)
            user = await repo.create(name="Test", email="test@example.com")
            await session.flush()
            user_id = user.id
            
            # Try to update non-existent user
            other_user = MockUser(name="Other", email="other@example.com")
...
def test_count_basic():
    ...
    async def _test():
        ...
        async with async_session() as session:
            ...
            # Count filtered
            count = await repo.count(name="User0")
            assert count == 1
    ...
    async def _test():
        ...
        async with async_session() as session:
            ...
            users = [
                ...
                MockUser(name="Bob", email="bob@example.com"),
                MockUser(name="Alice", email="alice2@example.com"),
                MockUser(name="Bob", email="bob2@example.com"),
                MockUser(name="Charlie", email="charlie@example.com"),
            ]
            session.add_all(users)
            await session.flush()
            ...
            # Count Bob
            count = await repo.count(name="Bob")
            assert count == 2
            
            # Count non-existent
            count = await repo.count(name="Unknown")
            assert count == 0
...
def test_multiple_creates_and_reads():
    ...
    async def _test():
        ...
        async with async_session() as session:
            ...
            repo = MockUserRepository(session)
            for i in range(5):
                user = await repo.create(name=f"User{i}", email=f"user{i}@example.com")
                await session.flush()
                assert user.id is not None
